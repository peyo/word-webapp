...
// formatter used across all APIs (excluding Lyrics API)
...
function displayDictAPI(dictData) {
  ...
  $("#def-word").append(
    `<b>${lower}</b> <i>(part of speech: ${dictData[0].partOfSpeech})</i>`
  );
...
function displayBookAPI(bookData) {
  ...
  if (bookData.items[0].volumeInfo.imageLinks.thumbnail === null) {
    ...
    $("#book-cover")
      .append(`<br><br><img class="book-border" src="${bookData.items[0].volumeInfo.imageLinks.thumbnail}"><br><br>
        ${bookData.items[0].volumeInfo.title}<br>
        By ${bookData.items[0].volumeInfo.authors[0]}<br><br>
        <a id="buy" href="${bookData.items[0].saleInfo.buyLink}" target="_blank">Buy/View</a>`);
  }
...
function searchYT(word) {
  if (word === null) {
    $("#yt-container").empty();
    $("#yt-container").append(
      "Since no lyrics were found. No videos were found."
    );
  } else {
    ...
    request.execute(response => {
      ...
      $("#yt-container").append(
        `<iframe src="${embedLink}" frameborder="0" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>`
      );
    });
  }
}
